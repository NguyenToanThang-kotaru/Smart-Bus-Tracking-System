...
import eye from "@/assets/Icon/Eye_yellow.png";
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex items-center justify-center gap-3 w-full">
        {/* Nút mũi tên trái */}
        <button className="flex-shrink-0 bg-white shadow rounded-full p-1 hover:scale-110 transition-transform cursor-pointer">
          <img src={left} alt="Left Arrow" className="w-6 h-6" />
        </button>

        {/* Bảng */}
        <div className="overflow-x-auto flex-1">
          <TableSchedule week={0}/>
        </div>
...
function TableSchedule({ week }) {
  ...
  const getScheduleForCell = (dayDate, shift) => {
    return schedule.filter((s) => {
      ...
      return dateStr === dayDate.toDateString() && timeStr >= shiftStart && timeStr < shiftEnd;
    });
  };
          ...
          {shifts.map((shift) => (
            <tr key={shift.id}>
              <td className="border font-semibold py-4">
                {shift.name}<br />
                <span className="text-gray-600 text-xs">({shift.start} - {shift.end})</span>
              </td>
                    ...
                    {cellData.length > 0 ? (
                      cellData.map((s) => (
                        <div key={s.MaLT} className="flex flex-col items-center gap-1">
                          ...
                          <div onClick={() => handleView(s)} className="flex w-2/3 pt-0.5 pb-0.5 rounded mt-2 justify-center bg-[rgb(11,35,71)] cursor-pointer hover:opacity-80">
