    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      ...
      >
        ...
        <div className="flex flex-col gap-y-2">
          <label className="text-xl text-mainBlue font-bold">Mã tuyến đường</label>
          <input
            type="text"
            value={MaTD}
            readOnly
        ...
        {/* Tên tuyến */}
        <div className="flex flex-col gap-y-2">
          <label className="text-xl text-mainBlue font-bold">Tên tuyến đường</label>
          <input
            type="text"
            value={TenTD}
        ...
        <div className="flex flex-col gap-y-2">
          <div className="flex items-center justify-between">
            <label className="text-xl text-mainBlue font-bold">Trạm thuộc tuyến</label>
            {!isView && (
              <button
                type="button"
                className="cursor-pointer hover:scale-95 border-2 rounded-[10px] border-mainYellow text-mainYellow font-semibold px-3 py-1 transition flex gap-x-[10px] items-center text-[12px]"
                onClick={() => setShowStationPicker(true)}
              >
                Chọn trạm
                <img src={addStation} alt="addStation" className="w-4 h-4" />
              </button>
            )}
          </div>
          <div className="border-2 rounded-[10px] p-2 w-full max-h-[100px] overflow-y-auto ">
            <Table
      ...
      {showStationPicker && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-[20px] p-6 w-[600px] max-h-[80vh] overflow-y-auto">
            <h3 className="text-xl font-bold text-mainBlue mb-4">Chọn trạm</h3>
